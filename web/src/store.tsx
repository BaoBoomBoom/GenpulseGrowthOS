...
function generatePlatformCopy(
  ...
  if (platform === "TikTok") {
    return {
      ...
      caption: `${topic.title} — ${topic.cta_type}`,
...
>[] = [
  {
    title: "Magnesium vs melatonin — wrong sleep problem?",
  ...
  {
    title: "Hair shed after your period — not shampoo betrayal",
...
function agentReply(agent: AgentId, userText: string): string {
  ...
  if (agent === "creative") {
    ...
    return "I am style and expression — tone, format, visual language, and multi-platform rewrites. Not a caption dump.";
  }
  if (t.includes("allocat") || t.includes("week") || t.includes("next")) {
    return "Next-week allocation should overweight CEO TikTok dating-biology and Genpulse sleep-signal threads — they complete the download?upload loop.";
  }
  if (t.includes("anomal")) {
    return "Anomaly: Lushair peptide posts get saves but weak Hair Scan CTR. Soften CTA ladder.";
  }
  return "I attribute business outcomes, spot trends/anomalies, and allocate next-week content capacity. Dashboards are inputs — decisions are the job.";
}
  ...
  scientist: [
    {
      ...
      agent: "scientist",
      role: "agent",
      text: "AI Scientist online. I guard truth and credibility — evidence extraction, controversy/risk flags, content-language translation, evidence scores.",
      created_at: nowIso(),
    },
  ],
  creative: [
  ...
  growth: [
    {
      ...
      agent: "growth",
      role: "agent",
      text: "AI Growth Manager online. I attribute, detect anomalies, recommend, and allocate next-week resources against downloads / uploads / scans — not vanity views.",
      created_at: nowIso(),
    },
  ],
};
      ...
      const created: ContentAsset[] = platforms.map((platform) => {
        ...
        return {
          ...
          creative_direction: brand
            ? `${brand.tone} — format ${topic.format_hint}`
            : topic.format_hint,
    ...
    return {
      ...
      scientistIngest(input) {
        ...
        const item: KnowledgeItem = {
          ...
          controversy:
            score < 70
              ? "Evidence base incomplete — treat claims as provisional."
              : undefined,
          risk_flags:
            score < 75
      ...
      creativeBriefFromTopic(topicId, brand) {
        ...
        const brief: CreativeBrief = {
          ...
          visual_language: `${b.tone} — ${b.prompt_beginning.slice(0, 80)}`,
          platform_plays: Object.fromEntries(
            topic.platform.map((p) => [
              p,
              p === "TikTok"
                ? "Hook-first — 3 beats — soft CTA"
                : p === "Instagram"
                  ? "Cover headline — 5-6 slides — aesthetic lock"
                  : p === "X"
                    ? "Thread ladder — evidence line — CTA"
                    : p === "LinkedIn"
                      ? "Insight open — structured body — founder close"
                      : `${p} native expression`,
            ])
          ),
      ...
      growthAllocateWeek() {
        const allocation: GrowthInsight = {
          ...
          detail:
            "CEO TikTok x6 (dating-biology) — Genpulse X/TikTok x8 (sleep signals + upload CTA) — Lushair IG/TT x5 (cycle-shed education ? Hair Scan) — Finegyno TT x4 — pause Dental until voice pack QA.",
          impact: "high",
        };
        ...
        setContentTasks((prev) =>
          prev.map((t) => {
            ...
            if (t.status !== "approved" && t.status !== "scheduled") {
              // hard gate — ignore illegal publish
              return t;
            }
      ...
      createDeal(input) {
        ...
        const deal: Deal = {
          ...
          last_contacted_on: input?.last_contacted_on ?? null,
          recommendation_blocker: input?.recommendation_blocker ?? null,
          expected_talk_at: input?.expected_talk_at ?? null,
      ...
      createOutreachDraft(leadId) {
        ...
        const body = isSalon
          ? `Hi ${lead.full_name.split(" ")[0]} — saw ${company?.name || "your salon"} investing in retention. Genpulse/Lushair can pair Hair Scan hardware with CRM tracking for client data continuity. Draft only — human must approve send.`
          : `Hi ${lead.full_name.split(" ")[0]} — researching ${company?.name || "your team"} against our ${lead.icp_type} ICP. Happy to share white-label / API options. Draft only — human must approve send.`;
        const act: Activity = {
          activity_id: uid("act"),
          lead_id: leadId,
          type: "outreach_draft",
